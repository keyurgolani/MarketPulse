...
describe('FooterSection', () => {
  ...
  it('has proper title styling', () => {
    ...
    expect(title).toHaveClass('text-sm', 'font-semibold', 'uppercase', 'tracking-wider');
  });
...
describe('FooterLink', () => {
  ...
  it('renders external link', () => {
    render(<FooterLink href="https://example.com" external>External Link</FooterLink>);
  ...
  it('shows external link icon', () => {
    render(<FooterLink href="https://example.com" external>External</FooterLink>);
  ...
  it('has proper styling', () => {
    ...
    const link = screen.getByRole('link');
    expect(link).toHaveClass('text-sm', 'text-gray-600', 'hover:text-gray-900', 'transition-colors', 'block');
  });

  it('applies custom className', () => {
    render(<FooterLink href="/test" className="custom-link">Link</FooterLink>);
...
describe('FooterCopyright', () => {
  it('renders with current year by default', () => {
    ...
    const copyright = screen.getByText(`© ${currentYear} Test Company. All rights reserved.`);
  ...
  it('renders with custom year', () => {
    ...
    const copyright = screen.getByText('© 2020 Test Company. All rights reserved.');
    expect(copyright).toBeInTheDocument();
  });
  ...
  it('has proper styling', () => {
    ...
    const container = screen.getByText(/© \d+ Test\. All rights reserved\./).parentElement;
    expect(container).toHaveClass('border-t', 'border-gray-200', 'pt-6', 'mt-8');
...
describe('FooterSocialLinks', () => {
  ...
  it('has proper styling', () => {
    ...
    const container = screen.getByTestId('twitter-icon').parentElement?.parentElement?.parentElement;
    expect(container).toHaveClass('flex', 'space-x-4');
    
    const link = screen.getByRole('link', { name: /follow us on twitter/i });
    expect(link).toHaveClass('text-gray-400', 'hover:text-gray-500', 'transition-colors');
  });
